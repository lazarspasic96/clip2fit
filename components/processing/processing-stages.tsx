    ...
    <View className="flex-1 items-center pt-12">
      ...
      <View
        style={{
          ...
          right: 0,
          height: 300,
          experimental_backgroundImage: 'radial-gradient(circle at 50% 30%, rgba(132,204,22,0.06) 0%, transparent 60%)',
        }}
        pointerEvents="none"
      />
      ...
      <Animated.View entering={FadeIn.duration(300)}>
        <HeroProgressRing progress={state.progress} platform={state.platform} stage={state.stage} />
      </Animated.View>

      {/* Stage message + URL */}
      <View className="mt-8 items-center gap-1 px-6">
        <Text className="text-lg font-inter-semibold text-content-primary text-center">{state.message}</Text>
      ...
      <View className="mt-10 self-stretch px-6">
        {STAGE_LABELS.map(({ key, label }, index) => (
          <StageIndicator
            ...
            label={label}
            index={index}
            status={state.stage === 'complete' ? 'completed' : getStageStatus(key, state.stage)}
          />
        ))}
      </View>
    </View>
