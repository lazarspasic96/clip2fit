import type {
  StatsMuscleGroupDistributionPoint,
  StatsPRExercise,
  StatsPRs,
  StatsPRTimelinePoint,
  StatsSummary,
  StatsTopExercise,
  StatsWeeklyFrequencyPoint,
} from '@/types/stats'
...
export const mapSummaryResponse = (api: unknown): StatsSummary => {
  ...
  const weeklyFrequency: StatsWeeklyFrequencyPoint[] = rawWeekly.map((item) => {
  ...
  const topExercises: StatsTopExercise[] = rawTopExercises.map((item) => {
    const row = asRecord(item)
  ...
  const muscleGroupDistribution: StatsMuscleGroupDistributionPoint[] = mappedMuscles
    .map((row) => ({
